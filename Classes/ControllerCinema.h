...
/* ************************  FUNCION�RIOS    *********************************/
	bool addEmployee(Funcionario f, int id) {
		//Verificando se login j� foi usado
		for (int i = 0; i < funcionarioAmount; i++)
		{
		...
		//Adicionando ao vetor de funcion�rios
		for (int i = 0; i < funcionarioAmount; i++) {
	...
	bool removeMovieSession(Sessao s) {
		for (int i = 0; i < sessaoAmount; i++) {
			if (sessoes[i].getIdSessao() == s.getIdSessao()) {
				isSalaValid[i] = false;
				return true;
			}
		}
	...
	static const int getFilmeAmount() {
		return filmeAmount;
	}

	static const int getFuncionarioAmount() {
		return funcionarioAmount;
	}

	static const int getSalaAmount() {
		return salaAmount;
	}

	static const int getSessaoAmount() {
		return sessaoAmount;
	}

	const Filme *getFilmes() const {
		return filmes;
	}

	const Funcionario *getFuncionarios() const {
		return funcionarios;
	}

	const Sala *getSalas() const {
		return salas;
	}

	const Sessao *getSessoes() const {
		return sessoes;
	}

	const bool *getIsFilmeValid() const {
		return isFilmeValid;
	}

	const bool *getIsFuncionarioValid() const {
		return isFuncionarioValid;
	}

	const bool *getIsSalaValid() const {
		return isSalaValid;
	}

	const bool *getIsSessaoValid() const {
		return isSessaoValid;
	}

	const int *getFilmeId() const {
		return filmeId;
	}

	const int *getFuncionarioId() const {
		return funcionarioId;
	}

	const int *getSalaId() const {
		return salaId;
	}

	const int *getSessaoId() const {
		return sessaoId;
	}
};
